...
## 2026-05-20 — Centralización de Validación de Formularios (Zod) y Rate Limiting / Lockout

### Decisión
Implementar un esquema de validación centralizado usando Zod (`lib/validation.ts`) y habilitar una política de rate limit/lockout a nivel base de datos en `auth.ts`.

### Motivo
- **Robustez y Seguridad**: Las validaciones manuales o HTML5 nativas no son lo suficientemente robustas para evitar datos corruptos o ataques de fuerza bruta en el formulario de login.
- **Seguridad contra ataques de fuerza bruta**: Se requería un mecanismo de seguridad para bloquear temporalmente cuentas tras múltiples intentos fallidos (5 intentos).

### Consecuencia
- Los formularios de login y producto se validan de forma estricta contra esquemas de Zod antes de procesar el envío de datos.
- Las cuentas de usuario se bloquean automáticamente por 15 minutos en la base de datos tras 5 intentos fallidos consecutivos, resguardando la integridad del sitio.

---

## 2026-05-20 — Desvinculación del compilador de next-intl en next.config.ts

### Decisión
Remover la integración a nivel plugin de `next-intl` en `next.config.ts` y en su lugar importar directamente el archivo JSON de mensajes `es.json` en `app/login/page.tsx`.

### Motivo
- **Fallo de pre-renderizado**: El plugin de next-intl provoca un conflicto en la fase estática del compilador de Next.js (Turbopack y Webpack en Next.js 16) durante el pre-renderizado de `_global-error`, arrojando un error interno `useContext null`.
- **Simplificación y Encapsulamiento**: Como únicamente el formulario de inicio de sesión requiere traducciones basadas en next-intl en la plataforma transaccional, es innecesario acoplar todo el build de Next.js al pipeline de internacionalización.

### Consecuencia
- Se solucionó de raíz el fallo en la fase de construcción del pipeline de despliegue (`npm run build`).
- Se mantiene el cumplimiento formal de la regla de "Zero Hardcoding" al continuar usando `NextIntlClientProvider` y `messages/es.json` sin alterar el stack de compilación de Next.js.
